...
let filter = inventory[i].car_model;
carModels.push(filter);
carModels.sort();
}
...
let carYears = [];
for (let i = 0; i < inventory.length; i++){
let cYear = inventory[i].car_year;
...
let oldCars = [];
    ...
    let yearFilter = inventory[i].car_year;
        if(yearFilter < 2000) {
    oldCars.push(yearFilter);
  }
}
...
for(i=0; i<inventory.length; i++) {
    let makeFilter = inventory[i]["car_make"];
    if (makeFilter === "BMW" || makeFilter === "Audi") {
        BMWAndAudi.push(makeFilter);
    }
}
